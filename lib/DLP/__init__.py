...
by Grosof, B. et.al. ("Description Logic Programs: Combining Logic Programs
with Description Logic" [1]) in section 4.4. As such, it implements recursive
mapping functions "T", "Th" and "Tb" which result in "custom" (dynamic)
rulesets, RIF Basic Logic Dialect: Horn rulesets [2], [3]. The rulesets are
evaluated against an efficient RETE-UL network.
...
__all__ = [
    ...
    'generatorFlattener',
    'handleConjunct',
    'IsaBooleanClassDescription',
    'IsaFactFormingConclusion',
    'IsaRestriction',
    'iterCondition',
    ...
    'MapDLPtoNetwork',
    'NormalizeBooleanClassOperand',
    'NormalizeClause',
    'PrepareHeadExistential',
    'PrepareHornClauseForRETE',
    'reduceAnd',
...
{?C rdfs:subClassOf ?SC. ?SC rdfs:subClassOf ?C
    } => {?C owl:equivalentClass ?SC}.

{?C owl:disjointWith ?B. ?M a ?C. ?Y a ?B } => {?M owl:differentFrom ?Y}.

{?P owl:inverseOf ?Q. ?P a owl:InverseFunctionalProperty
    } => {?Q a owl:FunctionalProperty}.
{?P owl:inverseOf ?Q. ?P a owl:FunctionalProperty
    } => {?Q a owl:InverseFunctionalProperty}.

#For OWL/InverseFunctionalProperty/premises004
{?C owl:oneOf ?L. ?L rdf:first ?X; rdf:rest rdf:nil. ?P rdfs:domain ?C
    } => {?P a owl:InverseFunctionalProperty}.
#For OWL/InverseFunctionalProperty/premises004
{?C owl:oneOf ?L. ?L rdf:first ?X; rdf:rest rdf:nil. ?P rdfs:range ?C
...
class Clause(OriginalClause):
    ...
    In addition, since we allow (in definite Horn) entailments beyond simple
    facts we ease restrictions on the form of the head to include Clauses
        """
...
def makeRule(clause, nsMap):
    ...
    for child in clause.head:
        ...
        vars.update([term for term in child.toRDFTuple(
    ...
    return Rule(clause, declare=vars,
                nsMapping=nsMap, negativeStratus=negativeStratus)
...
def MapDLPtoNetwork(network,
    ...
    for horn_clause in T(
        ...
        fullReduce = True
        for tx_horn_clause in \
            LloydToporTransformation(horn_clause, fullReduce):
            ...
            if len(disj) > 0:
                ...
                # print("No Disjunction in the body")
                for hc in ExtendN3Rules(network, NormalizeClause(
                                    tx_horn_clause), constructNetwork):
                    if safety in [DATALOG_SAFETY_LOOSE, DATALOG_SAFETY_STRICT]:
...
def IsaFactFormingConclusion(head):
    """
    Relative to the def-Horn ruleset, the def-LP is thus sound; moreover, it
    is complete for fact-form conclusions, i.e., for queries whose answers
    amount to conjunctions of facts. However, the def-LP is a mildly weaker
    version of the def-Horn ruleset, in the following sense. Every conclusion
    of the def-LP must have the form of a fact. By contrast, the entailments,
    i.e., conclusions, of the def-Horn ruleset are not restricted to be
    facts.' - Scan depth-first looking for Clauses
    """
...
def breadth_first(condition, children=traverseClause):
    ...
    From: http://code.activestate.com/recipes/231503/
    ...
    if IsaFactFormingConclusion(horn_clause.head):
        ...
        if constructNetwork:
            ...
            network.buildNetwork(iter(ruleStore.formulae[lhs]),
                                 ...
                                 Rule(horn_clause))
            network.alphaNodes = [node for node in list(
                network.nodes.values()) if isinstance(node, AlphaNode)]
...
def generatorFlattener(gen):
    ...
    i = list(gen)
    i = len(i) > 1 and [hasattr(
        i2, 'next') and generatorFlattener(i2) or i2 for i2 in i] or i[0]
    if hasattr(i, 'next'):
        # @FIXME: unused code
        ...
        i.formulae = [hasattr(
            i2, 'next') and generatorFlattener(i2) or i2 for i2 in i.formulae]
        # print(i)
        return i
    else:
        return i
...
def Tc(owlGraph, negatedFormula):
    """
    Handles the conversion of negated DL concepts into a general logic
    programming condition for the body of a rule that fires when the
    body conjunct is in the minimal model
    """
...
def handleConjunct(conjunction, owlGraph, o, conjunctVar=Variable('X')):
    for bodyTerm in Collection(owlGraph, o):
        ...
        if negatedFormula:
            ...
            if isinstance(
                normalizedBodyTerm, URIRef) and \
                normalizedBodyTerm.find(SKOLEMIZED_CLASS_NS) == -1:
                ...
                raise MalformedDLPFormulaError(
                    "Universal restrictions can only be used as the " +
                    "second argument to rdfs:subClassOf (GCIs)")
...
def T(owlGraph, complementExpansions=[], derivedPreds=[]):
    """
    #Subsumption (purely for TBOX classification)
    {?C rdfs:subClassOf ?SC. ?A rdfs:subClassOf ?C
                            } => {?A rdfs:subClassOf ?SC}.
    {?C owl:equivalentClass ?A
            } => {?C rdfs:subClassOf ?A. ?A rdfs:subClassOf ?C}.
    {?C rdfs:subClassOf ?SC. ?SC rdfs:subClassOf ?C
            } => {?C owl:equivalentClass ?SC}.
    ...
    to the DL (unary predicate logic) subsumption axiom described
    via rdfs:subClassOf
    """
        ...
        try:
            if s not in complementExpansions:
                if s in derivedPreds:
                    import warnings
                    warnings.warn(
                        "Derived predicate (%s) is defined via a " +
                        "conjunction (consider using a complex GCI) " % \
                        owlGraph.qname(s),
                        SyntaxWarning,
                        3)
                elif isinstance(s, BNode):
                    # and (None,None,s) not in owlGraph:# and \
                ...
                if isinstance(s, URIRef):
                    # S(?X) => O1 ^ O2 ^ ... ^ On
                    # special case, owl:intersectionOf is a neccessary and
                    # sufficient criteria and should thus work in *both*
                    # directions. This rule is not added for anonymous
                    # classes or derived predicates
...
def LloydToporTransformation(clause, fullReduction=True):
    ...
    if isinstance(clause.body, Or):
        ...
        (isinstance(clause.head, Exists) and
         isinstance(clause.head.formula, And)
...
def Th(owlGraph, _class, variable=Variable('X'), position=LHS):
    """
    DLP head (antecedent) knowledge assertional forms (ABox assertions,
    conjunction of ABox assertions, and universal role restriction assertions)
    """
    ...
    if OWL_NS.allValuesFrom in props:
        ...
        for s, p, o in owlGraph.triples((_class, OWL_NS.allValuesFrom, None)):
            prop = list(owlGraph.objects(
                subject=_class, predicate=OWL_NS.onProperty))[0]
            newVar = Variable(BNode())
            body = Uniterm(
        ...
        yield And(
                [first(Th(owlGraph, h, variable))
                        for h in BooleanClass(_class)])
    else:
        # Simple class
        yield Uniterm(RDF.type, [
            variable, isinstance(_class, BNode) and \
                SkolemizeExistentialClasses(_class) or _class],
                      newNss=owlGraph.namespaces())

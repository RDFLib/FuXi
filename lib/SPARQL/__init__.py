...
from rdflib import BNode, Literal, RDF, URIRef, Variable
...
def normalizeBindingsAndQuery(vars, bindings, conjunct):
    ...
    return list(_vars), conjunct, \
        project(bindings, _vars, inverse=True
                ) if appliedBindings else bindings
...
def normalizeUri(rdfTerm, revNsMap):
    ...
    prefix) or (unlike compute_qname) the Notation 3 form for URIs:
    <...URI...>
    """
    try:
...
def renderTerm(graph, term, predTerm=False, specialBNodeHandling=None):
    if term == RDF.type and predTerm:
        ...
        qname = normalizeUri(
            term, hasattr(graph, 'revNsMap') and graph.revNsMap or
        ...
        try:
            if isinstance(term, BNode):
                return term.n3() if specialBNodeHandling is None \
                    else specialBNodeHandling[0](term)
            else:
...
def RDFTuplesToSPARQL(conjunct,
    ...
    if symmAtomicInclusion:
        ...
        subquery = queryShell % (
            queryType, ' .\n'.join([
                '\t' + tripleToTriplePattern(edb, lit, specialBNodeHandling)
                                   for lit in conjunct]))
    return subquery
...
def RunQuery(subQueryJoin,
    ...
    if bool(bindings):
        ...
        openVars, conjGroundLiterals, bindings = \
            normalizeBindingsAndQuery(set(vars), bindings, subQueryJoin)
        vars = list(openVars)
    else:
            ...
            or (dict([
                    (vars[0],
                     specialBNodeHandling[-1](v)
                        if specialBNodeHandling
                     and isinstance(v, BNode) else v)
            ]) for v in rt)
        if debug:
            _debug("%s%s-> %s" % (
                subquery,
                projectedBindings and
                " %s apriori binding(s)" %
                len(projectedBindings) or '',
                rt and '[]')  # .. %s answers .. ]'%len(rt) or '[]')
            )
        return subquery, rt


def EDBQueryFromBodyIterator(
        factGraph, remainingBodyList, derivedPreds, hybridPredicates=None):
...
class ConjunctiveQueryMemoize(object):
    ...
    graph and a conjunctive query and returns a generator over results of
    evaluating the conjunctive query against the graph
    """
...
class EDBQuery(QNameManager, SetOperator, Condition):
    """
    A list of frames (comprised of EDB predicates) meant for evaluation over a
    large EDB
    ...
    def __init__(self,
        ...
        if bool(bindings):
            ...
            if returnVars is None:
                ...
                self.returnVars = (
                    returnVars if isinstance(returnVars, list)
    ...
    def accumulateBindings(self, bindings):
        """
        """
        self.bindings.update(project(
            bindings, self.getOpenVars(), inverse=True))
    ...
    def evaluate(self, debug=False, symmAtomicInclusion=False):
        ...
        for attempt in range(10):
            try:
                ...
                # warnings.warn(
                # "On attempt %s Received HTTP response" +
                # "code %s from server: %s" % (
                #     attempt+1,
                #     e.code,
                #     responseMsg),
                #     RuntimeWarning, 2)
                ...
                # we tried, and we had no failure, so
                break
        else:
            ...
            raise RuntimeError(
                ...
                "attempts to evaluate %s reached." %
                (self))

    def asSPARQL(self):
        # initialNs = hasattr(
        #    self.factGraph,'nsMap') and self.factGraph.nsMap or \
        #             dict([(k,v) for k,v in self.factGraph.namespaces()])
        return RDFTuplesToSPARQL(
                        self.formulae,
                        self.factGraph,
                        not self.returnVars,
                        self.returnVars,

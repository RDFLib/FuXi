...
def manchesterSyntax(thing, store, boolean=None, transientList=False):
    ...
    if compl:
        ...
        for boolProp, col in store.query(
            "SELECT ?p ?bool WHERE {" + \
            "?class a owl:Class; ?p ?bool . ?bool rdf:first ?foo }",
            processor="sparql",
            initBindings={
...
class AnnotatableTerms(Individual):
    ...
    """
    def __init__(self,
            identifier, graph=None, nameAnnotation=None, nameIsLabel=False):
    ...
    for kind in graph.objects(subject=classOrIdentifier(c),
        ...
        if kind == OWL_NS.Restriction:
            ...
            if not set([str(i.split(OWL_NS)[-1])
                for i in Restriction.restrictionKinds]).intersection(kwArgs):

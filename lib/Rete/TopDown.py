...
from rdflib import BNode, RDF, URIRef, Variable
...
def normalizeUri(rdfTerm, revNsMap):
    ...
    prefix) or (unlike compute_qname) the Notation 3 form for URIs: <...URI...>
    """
    try:
...
def renderTerm(graph, term):
    if term == RDF.type:
        ...
        qname = normalizeUri(term, hasattr(graph, 'revNsMap')
                             and graph.revNsMap
...
def invokeRule(priorAnswers,
    ...
    if lazyGenerator.successful:
        ...
        if not success:
            ...
            # yield False,_InferenceStep(
            #    step.parent,step.rule,source=step.source)
    else:
        lazyGenerator = lazyGeneratorPeek(lazyGenerator)
        projectedBindings = lazyGenerator.successful \
            and first(lazyGenerator) or {}

        # First we check if we can combine a large group of subsequent body
        # literals into a single query
        ...
        if len(conjGroundLiterals) > 1:
            ...
            queryConj = EDBQuery([copy.deepcopy(lit)
                                  for lit in conjGroundLiterals],
                                 factGraph,
                                 openVars,
                                 projectedBindings)
            ...
            if isinstance(answers, bool):
                ...
                if projectedBindings:
                    combinedAnswers = (mergeMappings1To2(
                        ans,
                        projectedBindings,
                        makeImmutable=True)
                        for ans in answers)
                    ...
                    combinedAnswers = (
                        MakeImmutableDict(ans) for ans in answers)
            ...
            if not rtCheck:
                ...
                for rt, _step in invokeRule(
                    ...
                    iter(remainingBodyList[
                         len(conjGroundLiterals):]),
                    sip,
                    otherargs,
                    isinstance(answers, bool),
                    step,
            ...
            try:
                ...
                # if a N3 builtin, execute it using given bindings for boolean
                # answer builtins are moved to end of rule when evaluating
                # rules via sip
                if isinstance(bodyLiteral, N3Builtin):
                    lhs = bodyLiteral.argument
                    rhs = bodyLiteral.result
                    lhs = isinstance(lhs, Variable) \
                        and projectedBindings[lhs] or lhs
                    rhs = isinstance(rhs, Variable) \
                        and projectedBindings[rhs] or rhs
                    assert lhs is not None and rhs is not None
                    if bodyLiteral.func(lhs, rhs):
                        if debug:
                            _debug("Invoked %s(%s,%s) -> True" % (
                                  bodyLiteral.uri, lhs, rhs))
                        # positive answer means we can continue processing
                        # the rule body
                        if buildProof:
                    ...
                    # For every body literal, subqueries are generated
                    # according to the sip
                    sipArcPred = URIRef(
                        GetOp(bodyLiteral) + '_' + '_'.join(
                            GetArgs(bodyLiteral)))
                    ...
                    for N, x in IncomingSIPArcs(sip, sipArcPred):
                        # That is, each subquery contains values for the bound
                        # arguments that are passed through the sip arcs
                        # entering the node corresponding to that literal
                    ...
                    if literalIsGround(subquery):
                        # subquery is ground, so there will only be boolean
                        # answers we return the conjunction of the answers
                        # for the current subquery
                        ...
                        if not answer and not bodyLiteral.naf or \
                            ...
                            # negative answer means the invokation of the rule
                            # fails either because we have a positive literal
                            # and there is no answer for the subgoal or the
                            # literal is negative and there is an answer for
                            # the subgoal
                            raise RuleFailure(
                            ...
                            if buildProof:
                                ...
                                step.antecedents.append(ns)
                            # positive answer means we can continue processing
                            # the rule body either because we have a positive
                            # literal and answers for subgoal or a negative
                            # literal and no answers for the the goal
                            for rt, _step in invokeRule(
                                               [projectedBindings],
                                               bodyLiteralIterator,
                                               sip,
                ...
                if priorBooleanGoalSuccess:
                    ...
                    # Return the most recent (cumulative) answers and the
                    # given step
...
def SipStrategy(query,
    ...
    Each evaluation uses memoization (via Python decorators) but also relies
    on well-formed rewrites for using semi-naive bottom up method over large
    SPARQL datasets.
    ...
    if queryLiteral in memoizeMemory:
        if debug:
            _debug("%sReturning previously calculated results for %s" %
                        ('\t' * proofLevel, queryLiteral))
        for answers in memoizeMemory[queryLiteral]:
            yield answers
    elif AlphaNode(goalRDFStatement
            ).alphaNetworkHash(True, skolemTerms=list(bindings.values())) in \
                    [AlphaNode(r.toRDFTuple()
                        ).alphaNetworkHash(
            ...
            for r in processedRules
                if AdornLiteral(goalRDFStatement).adornment ==
                   r.adornment]:
        if debug:
            _debug("%s Goal already processed..." % '\t' * proofLevel)
    else:
        ...
        # through the sip arcs entering the node corresponding to that literal.
        # For each subquery generated, there is a set of answers.
        answers = []

        # variableMapping = {}

        # Some TBox queries can be 'joined' together into SPARQL queries
        # against 'base' predicates via an RDF dataset.
        ...
        atomicInclusionAxioms = list(
            filter(IsAtomicInclusionAxiomRHS, rules))
        if atomicInclusionAxioms and len(atomicInclusionAxioms) > 1:
            if debug:
                from pprint import pformat
                _debug("\tCombining atomic inclusion axioms: ")
                _debug(pformat(atomicInclusionAxioms))
            if buildProof:
            ...
            if openVars:
                # mappings = {}
                # See if we need to do any variable mappings from the query
                # literals to the literals in the applicable rules
                ...
                # All the relevant derivations have been explored and the
                # result is a ground query we can directly execute against
                # the facts
                if buildProof:
        ...
        for rule in rules:
            ...
            if headBindings \
                and [term
                        for term in
                            rule.formula.head.getDistinguishedVariables(True)
                  ...
                  invokeRule([headBindings],
                    ...
                    if rt:
                        if isinstance(rt, dict):
                            ...
                            # correlation between the variables in the rule
                            # head and the variables in the original query
